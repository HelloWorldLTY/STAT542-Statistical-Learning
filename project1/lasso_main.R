...
test_y.dat <- read.csv("test_y.csv",stringsAsFactors = FALSE)
...
test.y <- log(test_y.dat$Sale_Price)
test.PID <- test.dat$PID
test.x <- subset(test.dat,select=-c(PID))
...
title <- data.frame(test.PID)
names(title) <- "PID"

pred1 <- data.frame(pred)
names(pred1) <- "Sale_Price"
pred1 <- cbind(title,pred1)

write.csv(pred1, "mysubmission1.txt", row.names = FALSE, quote = FALSE)
